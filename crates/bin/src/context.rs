...
use wazm_wasmi::{Config, ExternType, Func, FuncType, Instance, Module, Store};

/// The [`Context`] for the `runtime` CLI application.
///
...
impl Context {
    ...
    pub fn new(wasm_file: &Path, wasi_ctx: WasiCtx, fuel: Option<u64>) -> Result<Self, Error> {
        ...
        let module = wasmi::Module::new(&engine, &mut &wasm_bytes[..])
            .map_err(|error| anyhow!("failed to parse and validate Wasm module {wasm_file:?}: {error}"))?;
        let mut store = wasmi::Store::new(&engine, wasi_ctx);
        if let Some(fuel) = fuel {
            store
                .add_fuel(fuel)
                .unwrap_or_else(|error| panic!("error: fuel metering is enabled but encountered: {error}"));
        }
