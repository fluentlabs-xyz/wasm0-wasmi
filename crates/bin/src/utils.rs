...
pub fn read_wasm_or_wat(wasm_file: &Path) -> Result<Vec<u8>, Error> {
    let mut wasm_bytes = fs::read(wasm_file).map_err(|_| anyhow!("failed to read Wasm file {wasm_file:?}"))?;
    if wasm_file.extension().and_then(OsStr::to_str) == Some("wat") {
        ...
        wasm_bytes = wat2wasm(&wat).map_err(|error| anyhow!("failed to parse .wat file {wasm_file:?}: {error}"))?;
    }
    Ok(wasm_bytes)
}
...
///
/// The returned buffer can be used as function results for [`Func::call`](`runtime::Func::call`).
...
/// as function arguments for [`Func::call`][`runtime::Func::call`].
///
/// # Errors
///
    ...
    ty.params()
        ...
        .map(|(n, (param_type, arg))| {
            ...
            match param_type {
                ...
                ValueType::F32 => arg.parse::<f32>().map(F32::from).map(Value::from).map_err(make_err!()),
                ValueType::F64 => arg.parse::<f64>().map(F64::from).map(Value::from).map_err(make_err!()),
                ValueType::FuncRef => {
                    bail!("the runtime CLI cannot take arguments of type funcref")
                }
                ValueType::ExternRef => {
                    bail!("the runtime CLI cannot take arguments of type externref")
                }
            }
        })
