...
impl<'parser> FuncTranslator<'parser> {
    ...
    pub fn new(func: FuncIdx, res: ModuleResources<'parser>, alloc: FuncTranslatorAllocations) -> Self {
        Self {
            func,
            res,
            reachable: true,
    ...
    fn optimize_global_get(global_type: &GlobalType, init_value: Option<&ConstExpr>) -> Option<Instruction> {
        ...
        T: Into<Value>,
    {
        self.translate_if_reachable(|builder| {
            ...
            builder.stack_height.push();
            builder.alloc.inst_builder.push_inst(Instruction::const_i32(value));
            Ok(())
        })
    }
        ...
        T: Into<Value>,
    {
        self.translate_if_reachable(|builder| {
            ...
            let value = value.into();
            builder.stack_height.push();
            builder.alloc.inst_builder.push_inst(Instruction::const_i64(value));
            Ok(())
        })
    }
...
impl<'a> VisitOperator<'a> for FuncTranslator<'a> {
    ...
    fn visit_if(&mut self, block_type: wasmparser::BlockType) -> Result<(), TranslationError> {
        ...
        if self.is_reachable() {
            ...
            self.alloc.inst_builder.push_inst(Instruction::BrIfEqz(branch_params));
            let consume_fuel = self.is_fuel_metering_enabled().then(|| {
                self.alloc
                    .inst_builder
                    .push_inst(Instruction::consume_fuel(self.fuel_costs().base))
        ...
        self.translate_if_reachable(|builder| {
            match builder.acquire_target(relative_depth)? {
                AcquiredTarget::Branch(end_label, drop_keep) => {
                    ...
                    builder.alloc.inst_builder.push_inst(Instruction::Br(params));
                }
        ...
        self.translate_if_reachable(|builder| {
            ...
            match builder.acquire_target(relative_depth)? {
                AcquiredTarget::Branch(end_label, drop_keep) => {
                    ...
                    builder.bump_fuel_consumption(builder.fuel_costs().fuel_for_drop_keep(drop_keep));
                    let params = builder.branch_params(end_label, drop_keep);
                    builder.alloc.inst_builder.push_inst(Instruction::BrIfNez(params));
                }
