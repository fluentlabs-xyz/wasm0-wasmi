...
impl Stack {
    ...
    pub fn new(limits: StackLimits) -> Self {
        ...
        let values = ValueStack::new(limits.initial_value_stack_height, limits.maximum_value_stack_height);
        Self { values, frames }
    }
    ...
    pub fn prepare_wasm_call(&mut self, wasm_func: &WasmFuncEntity, code_map: &CodeMap) -> Result<(), TrapCode> {
    ...
    pub fn call_host_impl<T>(
        ...
        let (input_types, output_types) = func_types.resolve_func_type(host_func.ty_dedup()).params_results();
        ...
        let params_results = FuncParams::new(self.values.peek_as_slice_mut(max_inout), len_inputs, len_outputs);
        ...
        let trampoline = ctx
            ...
            .clone();
        trampoline.call(ctx, instance, params_results).map_err(|error| {
