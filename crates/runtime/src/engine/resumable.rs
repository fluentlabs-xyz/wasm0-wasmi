use super::Func;
use crate::{engine::Stack, func::CallResultsTuple, AsContextMut, Engine, Error, Value, WasmResults};
...
pub struct ResumableInvocation {
    ...
    /// # Note
    ///
    /// - This handle is required to resolve function types of both `func` and `host_func` fields as
    ///   well as in the `Drop` impl to recycle the stack.
    engine: Engine,
...
impl<Results> TypedResumableCall<Results> {
    ...
    pub(crate) fn new(call: ResumableCallBase<Results>) -> Self {
        match call {
            ...
            ResumableCallBase::Resumable(invocation) => Self::Resumable(TypedResumableInvocation::new(invocation)),
        }
    }
}
