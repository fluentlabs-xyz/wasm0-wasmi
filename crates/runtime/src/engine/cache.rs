...
impl InstanceCache {
    ...
    pub fn get_data_segment(&mut self, ctx: &StoreInner, index: u32) -> DataSegment {
        ...
        ctx.resolve_instance(self.instance())
            ...
            .unwrap_or_else(|| unreachable!("missing data segment ({index:?}) for instance: {instance:?}",))
    }
    ...
    #[inline]
    pub fn get_element_segment(&mut self, ctx: &StoreInner, index: ElementSegmentIdx) -> ElementSegment {
        let instance = self.instance();
        ctx.resolve_instance(self.instance())
            .get_element_segment(index.into_inner())
            .unwrap_or_else(|| unreachable!("missing element segment ({index:?}) for instance: {instance:?}",))
    }
    ...
    ) -> (&'a InstanceEntity, &'a mut TableEntity, &'a ElementSegmentEntity) {
    ...
    fn load_default_memory(&mut self, ctx: &StoreInner) -> &Memory {
        ...
        let default_memory = ctx
            ...
            .unwrap_or_else(|| unreachable!("missing default linear memory for instance: {instance:?}"));
    ...
    fn load_table_at(&mut self, ctx: &StoreInner, index: TableIdx) -> Table {
        let table = ctx
            ...
            .unwrap_or_else(|| unreachable!("missing table at index {index:?} for instance: {:?}", self.instance));
    ...
    fn load_func_at(&mut self, ctx: &StoreInner, index: FuncIdx) -> Func {
        let func = ctx
            ...
            .unwrap_or_else(|| unreachable!("missing func at index {index:?} for instance: {:?}", self.instance));
    ...
    /// If the currently used [`Instance`] does not have a [`Func`] at the index.
    #[inline(always)]
    fn get_global_mut<'ctx>(&mut self, ctx: &'ctx mut StoreInner, global_index: GlobalIdx) -> &'ctx mut UntypedValue {
    ...
    /// If the currently used [`Instance`] does not have a [`Func`] at the index.
    #[inline(always)]
    pub fn set_global(&mut self, ctx: &mut StoreInner, global_index: GlobalIdx, new_value: UntypedValue) {
