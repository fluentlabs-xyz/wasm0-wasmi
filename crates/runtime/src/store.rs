...
use wazm_arena::{Arena, ArenaIndex, GuardedEntity};
use wazm_core::TrapCode;
...
impl ArenaIndex for StoreIdx {
    ...
    fn from_usize(value: usize) -> Self {
        let value = value
            .try_into()
            .unwrap_or_else(|error| panic!("index {value} is out of bounds as store index: {error}"));
        Self(value)
    }
}
...
impl Fuel {
    ...
    pub fn sufficient_fuel(&self, delta: u64) -> Result<(), TrapCode> {
        self.remaining.checked_sub(delta).map(|_| ()).ok_or(TrapCode::OutOfFuel)
    }
    ...
    pub fn consume_fuel(&mut self, delta: u64) -> Result<u64, TrapCode> {
        self.remaining = self.remaining.checked_sub(delta).ok_or(TrapCode::OutOfFuel)?;
        Ok(self.remaining)
    }
}
...
impl StoreInner {
    ...
    pub(super) fn alloc_element_segment(&mut self, segment: ElementSegmentEntity) -> ElementSegment {
    ...
    fn resolve<'a, Idx, Entity>(&self, idx: &Stored<Idx>, entities: &'a Arena<Idx, Entity>) -> &'a Entity
    where
        Idx: ArenaIndex + Debug,
    {
    ...
    /// - If the [`DedupFuncType`] does not originate from this [`Store`].
    /// - If the [`DedupFuncType`] cannot be resolved to its entity.
    pub fn resolve_func_type_with<R>(&self, func_type: &DedupFuncType, f: impl FnOnce(&FuncType) -> R) -> R {
    ...
    /// - If the [`Table`] does not originate from this [`Store`].
    /// - If the [`Table`] cannot be resolved to its entity.
    pub fn resolve_table_pair_mut(&mut self, fst: &Table, snd: &Table) -> (&mut TableEntity, &mut TableEntity) {
        let fst = self.unwrap_stored(fst.as_inner());
        let snd = self.unwrap_stored(snd.as_inner());
        self.tables
            .get_pair_mut(fst, snd)
            .unwrap_or_else(|| panic!("failed to resolve stored pair of entities: {fst:?} and {snd:?}"))
    }
    ...
    /// - If the [`ElementSegment`] does not originate from this [`Store`].
    /// - If the [`ElementSegment`] cannot be resolved to its entity.
    pub fn resolve_element_segment_mut(&mut self, segment: &ElementSegment) -> &mut ElementSegmentEntity {
    ...
    pub fn resolve_func(&self, func: &Func) -> &FuncEntity {
        ...
        self.funcs
            .get(entity_index)
            .unwrap_or_else(|| panic!("failed to resolve stored Wasm or host function: {entity_index:?}"))
    }
}
...
impl<T> Store<T> {
    ...
    /// - If the [`Memory`] does not originate from this [`Store`].
    /// - If the [`Memory`] cannot be resolved to its entity.
    pub(super) fn resolve_memory_and_state_mut(&mut self, memory: &Memory) -> (&mut MemoryEntity, &mut T) {
