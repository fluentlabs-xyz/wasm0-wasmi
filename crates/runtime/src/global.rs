...
impl ArenaIndex for GlobalIdx {
    ...
    fn from_usize(value: usize) -> Self {
        let value = value
            .try_into()
            .unwrap_or_else(|error| panic!("index {value} is out of bounds as global index: {error}"));
        Self(value)
    }
}
...
impl Display for GlobalError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::TypeMismatch { expected, encountered } => {
                write!(
                    f,
            ...
            Self::UnsatisfyingGlobalType { unsatisfying, required } => {
                write!(
                    f,
...
impl Global {
    ...
    pub fn set(&self, mut ctx: impl AsContextMut, new_value: Value) -> Result<(), GlobalError> {
        ctx.as_context_mut().store.inner.resolve_global_mut(self).set(new_value)
    }
