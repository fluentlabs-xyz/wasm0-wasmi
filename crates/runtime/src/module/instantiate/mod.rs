use wazm_core::{Trap, UntypedValue};
...
impl Module {
    ...
    /// [`Func`]: [`crate::Func`]
    pub(crate) fn instantiate<I>(&self, mut context: impl AsContextMut, externals: I) -> Result<InstancePre, Error>
    where
        ...
        loop {
            ...
            match (import.ty(), external) {
                (ExternType::Func(expected_signature), Extern::Func(func)) => {
                    ...
                    let actual_signature = self.engine.resolve_func_type(actual_signature, FuncType::clone);
    ...
    fn extract_functions(
        ...
        for (func_type, func_body) in self.internal_funcs() {
            ...
            let func = context.as_context_mut().store.inner.alloc_func(wasm_func.into());
            builder.push_func(func);
        }
    }
    ...
    ///
    /// [`Store`]: struct.Store.html
    fn extract_memories(&self, context: &mut impl AsContextMut, builder: &mut InstanceEntityBuilder) {
        for memory_type in self.internal_memories().copied() {
            let memory = Memory::new(context.as_context_mut(), memory_type).unwrap_or_else(|error| {
                panic!(
    ...
    ///
    /// [`Store`]: struct.Store.html
    fn extract_globals(&self, context: &mut impl AsContextMut, builder: &mut InstanceEntityBuilder) {
        ...
        for (global_type, global_init) in self.internal_globals() {
            ...
            let global = Global::new(context.as_context_mut(), init_value.with_type(value_type), mutability);
            context
                .as_context_mut()
                .store
                .tracer
                .global_variable(init_value, global_index);
            global_index += 1;
            builder.push_global(global);
        }
    }
    ...
    fn eval_init_expr(context: impl AsContext, builder: &InstanceEntityBuilder, init_expr: &ConstExpr) -> UntypedValue {
        init_expr
            .eval_with_context(
    ...
    fn extract_exports(&self, context: &mut impl AsContextMut, builder: &mut InstanceEntityBuilder) {
        for (field, idx) in &self.exports {
            ...
            context
                ...
                .store
                .tracer
                .register_extern(external, field, entity_index);
    ...
    fn initialize_table_elements(
        ...
        for segment in &self.element_segments[..] {
            ...
            if let ElementSegmentKind::Active(active) = segment.kind() {
                let dst_index = u32::from(Self::eval_init_expr(&mut *context, builder, active.offset()));
    ...
    fn initialize_memory_data(
        ...
        for segment in &self.data_segments[..] {
            ...
            if let DataSegmentKind::Active(segment) = segment.kind() {
                ...
                let offset = u32::from(Self::eval_init_expr(&mut *context, builder, offset_expr)) as usize;
                ...
                context
                    ...
                    .store
                    .tracer
                    .global_memory(offset as u32, bytes.len() as u32, bytes);
            }
