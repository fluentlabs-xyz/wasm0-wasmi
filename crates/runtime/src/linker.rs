    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Self::DuplicateDefinition { import_name } => {
                write!(f, "encountered duplicate definition with name `{import_name}`",)
            }
            Self::MissingDefinition { name, ty } => {
                write!(f, "cannot find definition for import {name} with type {ty:?}",)
            }
            Self::InvalidTypeDefinition { name, expected, found } => {
                write!(
                    f,
                    "found definition for import {name} with type {expected:?} but found type {found:?}"
                )
            }
            Self::FuncTypeMismatch { name, expected, found } => {
                write!(
                    f,
            ...
            Self::InvalidTableSubtype { name, ty, other } => {
                write!(f, "import {name}: table type {ty:?} is not a subtype of {other:?}")
            }
            Self::InvalidMemorySubtype { name, ty, other } => {
                write!(f, "import {name}: memory type {ty:?} is not a subtype of {other:?}")
            }
            Self::GlobalTypeMismatch { name, expected, found } => {
                write!(
                    f,
    ...
    pub fn as_func(&self, mut ctx: impl AsContextMut<UserState = T>) -> Option<Func> {
        match self {
            ...
            Definition::HostFunc(host_func) => {
                ...
                let func = ctx.as_context_mut().store.inner.alloc_func(FuncEntity::Host(entity));
                Some(func)
            }
            _ => None,
        }
...
impl<'a, T> Debug for DebugHostFuncEntity<'a, T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.engine.resolve_func_type(self.host_func.ty_dedup(), |func_type| {
    ...
    fn process_import(
        ...
        match import.ty() {
            ExternType::Func(expected_type) => {
                let found_type = resolved.ty(&context).func().cloned().ok_or_else(invalid_type)?;
                if &found_type != expected_type {
                    return Err(LinkerError::func_type_mismatch(import_name, expected_type, &found_type))
                        .map_err(Into::into);
                }
                let func = resolved
                    .as_func(&mut context)
            ...
            ExternType::Table(expected_type) => {
                ...
                found_type
                    .is_subtype_or_err(expected_type)
                    .map_err(|_| LinkerError::table_type_mismatch(import_name, expected_type, &found_type))?;
            ...
            ExternType::Memory(expected_type) => {
                ...
                found_type
                    .is_subtype_or_err(expected_type)
                    .map_err(|_| LinkerError::invalid_memory_subtype(import_name, expected_type, &found_type))?;
...
mod tests {
    ...
    fn linker_funcs_work() {
        ...
        let a_init = 42;
        let b_init = 77;
        let mut store = <Store<HostState>>::new(&engine, HostState { a: a_init, b: b_init });
        let wat = r#"
                (module
        ...
        let wasm_get_a = instance.get_typed_func::<(), i32>(&store, "wasm_get_a").unwrap();
        let wasm_set_a = instance.get_typed_func::<i32, ()>(&store, "wasm_set_a").unwrap();
        let wasm_get_b = instance.get_typed_func::<(), i64>(&store, "wasm_get_b").unwrap();
        let wasm_set_b = instance.get_typed_func::<i64, ()>(&store, "wasm_set_b").unwrap();
