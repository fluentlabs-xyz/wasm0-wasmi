...
impl ArenaIndex for MemoryIdx {
    ...
    fn from_usize(value: usize) -> Self {
        let value = value
            .try_into()
            .unwrap_or_else(|error| panic!("index {value} is out of bounds as memory index: {error}"));
        Self(value)
    }
}
...
impl MemoryType {
    ...
    pub fn new(initial: u32, maximum: Option<u32>) -> Result<Self, MemoryError> {
        ...
        let maximum_pages = match maximum {
            Some(maximum) => Pages::new(maximum).ok_or(MemoryError::InvalidMemoryType)?.into(),
            None => None,
        };
        Ok(Self {
            initial_pages,
...
impl MemoryEntity {
    ...
    pub fn new(memory_type: MemoryType) -> Result<Self, MemoryError> {
        ...
        let initial_len = initial_pages.to_bytes().ok_or(MemoryError::OutOfBoundsAllocation)?;
        let memory = Self {
    ...
    pub fn grow(&mut self, additional: Pages) -> Result<Pages, MemoryError> {
        ...
        let new_size = new_pages.to_bytes().ok_or(MemoryError::OutOfBoundsAllocation)?;
...
impl Memory {
    ...
    pub(crate) fn dynamic_ty(&self, ctx: impl AsContext) -> MemoryType {
        ctx.as_context().store.inner.resolve_memory(self).dynamic_ty()
    }
    ...
    pub fn current_pages(&self, ctx: impl AsContext) -> Pages {
        ctx.as_context().store.inner.resolve_memory(self).current_pages()
    }
    ...
    ///
    /// Panics if `ctx` does not own this [`Memory`].
    pub fn grow(&self, mut ctx: impl AsContextMut, additional: Pages) -> Result<Pages, MemoryError> {
        ctx.as_context_mut()
            .store
            .inner
    ...
    ///
    /// Panics if `ctx` does not own this [`Memory`].
    pub fn data_and_store_mut<'a, T: 'a>(&self, ctx: impl Into<StoreContextMut<'a, T>>) -> (&'a mut [u8], &'a mut T) {
    ...
    ///
    /// Panics if `ctx` does not own this [`Memory`].
    pub fn read(&self, ctx: impl AsContext, offset: usize, buffer: &mut [u8]) -> Result<(), MemoryError> {
        ctx.as_context().store.inner.resolve_memory(self).read(offset, buffer)
    }
    ...
    ///
    /// Panics if `ctx` does not own this [`Memory`].
    pub fn write(&self, mut ctx: impl AsContextMut, offset: usize, buffer: &[u8]) -> Result<(), MemoryError> {
        ctx.as_context_mut()
            .store
            .inner
            .resolve_memory_mut(self)
