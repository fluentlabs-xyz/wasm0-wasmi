    ...
    Results: WasmResults,
{
    ...
    pub(crate) fn new(ctx: impl AsContext, func: Func) -> Result<Self, Error> {
        ...
        let (actual_params, actual_results) = (<Params as WasmTypeList>::types(), <Results as WasmTypeList>::types());
    ...
    Results: WasmResults,
{
    ...
    fn call_results(self, results: &[UntypedValue]) -> Self::Results {
        <Results as WasmTypeList>::from_values(results).expect("unable to construct typed results from call results")
    }
}
