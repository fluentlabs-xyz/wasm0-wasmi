...
type TrampolineFn<T> = dyn Fn(Caller<T>, FuncParams) -> Result<FuncFinished, Trap> + Send + Sync + 'static;
...
impl Func {
    ...
    pub(crate) fn ty_dedup<'a, T: 'a>(&self, ctx: impl Into<StoreContext<'a, T>>) -> &'a DedupFuncType {
    ...
    pub fn ty(&self, ctx: impl AsContext) -> FuncType {
        ctx.as_context().store.inner.resolve_func_type(self.ty_dedup(&ctx))
    }
    ...
    pub fn call<T>(
        ...
        ctx.as_context()
            ...
            .engine()
            .clone()
            .execute_func(ctx.as_context_mut(), self, inputs, outputs)?;
        Ok(())
    }
