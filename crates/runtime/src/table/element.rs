...
impl ArenaIndex for ElementSegmentIdx {
    ...
    fn from_usize(value: usize) -> Self {
        let value = value
            .try_into()
            .unwrap_or_else(|error| panic!("index {value} is out of bounds as element segment index: {error}"));
        Self(value)
    }
}
...
impl ElementSegment {
    ...
    pub fn new(mut ctx: impl AsContextMut, segment: &module::ElementSegment) -> Self {
        ...
        ctx.as_context_mut().store.inner.alloc_element_segment(entity)
    }

    /// Returns the number of items in the [`ElementSegment`].
    pub fn size(&self, ctx: impl AsContext) -> u32 {
        ctx.as_context().store.inner.resolve_element_segment(self).size()
    }
...
impl ElementSegmentEntity {
    ...
    pub fn items(&self) -> &[ConstExpr] {
        self.items.as_ref().map(ElementSegmentItems::items).unwrap_or(&[])
    }
