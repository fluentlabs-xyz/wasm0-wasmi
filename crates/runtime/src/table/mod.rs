...
use wazm_core::{TrapCode, UntypedValue, ValueType};
...
impl ArenaIndex for TableIdx {
    ...
    fn from_usize(value: usize) -> Self {
        let value = value
            .try_into()
            .unwrap_or_else(|error| panic!("index {value} is out of bounds as table index: {error}"));
        Self(value)
    }
}
...
impl TableEntity {
    ...
    pub fn get(&self, index: u32) -> Option<Value> {
        self.get_untyped(index).map(|untyped| self.make_typed(untyped))
    }
    ...
    pub fn set_untyped(&mut self, index: u32, value: UntypedValue) -> Result<(), TableError> {
        ...
        let untyped = self
            ...
            .ok_or(TableError::AccessOutOfBounds { current, offset: index })?;
        *untyped = value;
        Ok(())
    }
    ...
    pub fn copy_within(&mut self, dst_index: u32, src_index: u32, len: u32) -> Result<(), TrapCode> {
...
impl Table {
    ...
    pub(crate) fn dynamic_ty(&self, ctx: impl AsContext) -> TableType {
        ctx.as_context().store.inner.resolve_table(self).dynamic_ty()
    }
    ...
    ///
    /// Panics if `ctx` does not own this [`Table`].
    pub fn grow(&self, mut ctx: impl AsContextMut, delta: u32, init: Value) -> Result<u32, TableError> {
        ctx.as_context_mut()
            .store
            .inner
    ...
    ///
    /// Panics if `ctx` does not own this [`Table`].
    pub fn set(&self, mut ctx: impl AsContextMut, index: u32, value: Value) -> Result<(), TableError> {
        ctx.as_context_mut()
            .store
            .inner
            .resolve_table_mut(self)
    ...
    pub fn copy(
        ...
        if Self::eq(dst_table, src_table) {
            ...
            let table = store.as_context_mut().store.inner.resolve_table_mut(dst_table);
            table
            ...
            TableEntity::copy(dst_table, dst_index, src_table, src_index, len).map_err(|_| TableError::CopyOutOfBounds)
        }
    }
    ...
    ///
    /// [`Store`]: [`crate::Store`]
    pub fn fill(&self, mut ctx: impl AsContextMut, dst: u32, val: Value, len: u32) -> Result<(), TrapCode> {
        ctx.as_context_mut()
            .store
            .inner
            .resolve_table_mut(self)
