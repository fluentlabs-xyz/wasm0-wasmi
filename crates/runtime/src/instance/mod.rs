...
use super::{engine::DedupFuncType, AsContext, Func, Global, Memory, Module, StoreContext, Stored, Table};
use crate::{func::FuncError, memory::DataSegment, ElementSegment, Error, TypedFunc, WasmParams, WasmResults};
...
impl ArenaIndex for InstanceIdx {
    ...
    fn from_usize(value: usize) -> Self {
        let value = value
            .try_into()
            .unwrap_or_else(|error| panic!("index {value} is out of bounds as instance index: {error}"));
        Self(value)
    }
}
...
impl Instance {
    ...
    pub(crate) fn get_func_by_index(&self, store: impl AsContext, index: u32) -> Option<Func> {
        store.as_context().store.inner.resolve_instance(self).get_func(index)
    }
    ...
    pub fn get_export(&self, store: impl AsContext, name: &str) -> Option<Extern> {
        store.as_context().store.inner.resolve_instance(self).get_export(name)
    }
