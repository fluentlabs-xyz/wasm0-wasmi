...
use wazm_core::{Trap, TrapCode, ValueType};
use wazm_wasmi::{
    Config,
    Engine,
    Error,
...
fn resumable_call_smoldot_tail_01() {
    ...
    assert_eq!(
        wasm_fn.call_resumable(&mut store, ()).unwrap_err().i32_exit_status(),
        Some(100),
    );
}
...
fn resumable_call_host() {
    ...
    let host_fn = Func::wrap(&mut store, || -> Result<(), Trap> { Err(Trap::i32_exit(100)) });
    ...
    let error = host_fn.call_resumable(&mut store, &[], &mut []).unwrap_err();
    match error {
        Error::Trap(trap) => {
...
trait AssertResumable {
    ...
    fn assert_resumable(self, store: &Store<()>, exit_status: i32, host_results: &[ValueType]) -> Self::Invocation;
