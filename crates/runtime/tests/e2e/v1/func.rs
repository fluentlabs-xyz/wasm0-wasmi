//! Tests for the `Func` type in `runtime`.

use core::slice;

use assert_matches::assert_matches;
use wazm_core::{ValueType, F32, F64};
use wazm_wasmi::{errors::FuncError, Engine, Error, Func, FuncType, Store, Value};
...
fn dynamic_add2_works() {
    ...
    for a in 0..10 {
        for b in 0..10 {
            ...
            add2.call(&mut store, &params, slice::from_mut(&mut result)).unwrap();
...
fn setup_add3() -> (Store<()>, Func, Func) {
    ...
    let add3_dyn = Func::new(
        &mut store,
        FuncType::new([ValueType::I32, ValueType::I32, ValueType::I32], [ValueType::I32]),
        |_caller, inputs: &[Value], results: &mut [Value]| {
    ...
    for a in 0..5 {
        for b in 0..5 {
            for c in 0..5 {
                ...
                let mut result = Value::I32(0);
                // Call to Func with statically typed closure.
                add3.call(&mut store, &params, slice::from_mut(&mut result)).unwrap();
                assert_eq!(result.i32(), Some(expected));
                // Reset result before execution.
                result = Value::I32(0);
                // Call to Func with dynamically typed closure.
...
fn dynamic_duplicate_works() {
    ...
    for input in 0..10 {
        ...
        // Call to Func with dynamically typed closure.
        duplicate_dyn.call(&mut store, &params, &mut results).unwrap();
        assert_eq!(results[0].i32(), expected[0].i32());
        assert_eq!(results[1].i32(), expected[1].i32());
    }
}
...
fn static_duplicate_works() {
    ...
    for input in 0..10 {
        ...
        assert_eq!(duplicate_dyn.call(&mut store, input).unwrap(), (input, input));
    }
}
...
fn setup_many_params_many_results() -> (Store<()>, Func) {
    ...
    let func = Func::wrap(
        ...
        |v0: i32,
         ...
         v13: i32,
         v14: i32,
         v15: i32| { (v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15) },
    );
    (store, func)
}
...
fn dynamic_many_types_works() {
    ...
    let func = Func::wrap(&mut store, |v0: i32, v1: u32, v2: i64, v3: u64, v4: F32, v5: F64| {
        (v0, v1, v2, v3, v4, v5)
    });
...
fn dynamic_type_check_works() {
    ...
    assert_matches!(
        identity.call(&mut store, &[Value::I32(0)], &mut [Value::I32(0), Value::I32(1)],),
        Err(Error::Func(FuncError::MismatchingResultLen))
    );
    // Case: Mismatching type given as input to function.
    for input in &[Value::I64(0), Value::F32(0.0.into()), Value::F64(0.0.into())] {
        assert_matches!(
            identity.call(
                &mut store,
    ...
    assert_matches!(identity.call(&mut store, &[Value::I32(0)], &mut [Value::I64(0)]), Ok(_));
}
