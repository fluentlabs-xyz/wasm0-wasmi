...
use wazm_wasmi::{Config, StackLimits};

/// Returns the Wasm binary at the given `file_name` as `Vec<u8>`.
///
/// # Note
///
/// This includes validation and compilation to `runtime` bytecode.
///
/// # Panics
///
...
pub fn load_wasm_from_file(file_name: &str) -> Vec<u8> {
    let mut file =
        File::open(file_name).unwrap_or_else(|error| panic!("could not open benchmark file {}: {}", file_name, error));
    let mut buffer = Vec::new();
    file.read_to_end(&mut buffer)
        .unwrap_or_else(|error| panic!("could not read file at {} to buffer: {}", file_name, error));
    buffer
}
...
/// Parses the Wasm binary at the given `file_name` into a `runtime` module.
///
/// # Note
///
/// This includes validation and compilation to `runtime` bytecode.
...
pub fn load_module_from_file(file_name: &str) -> wasmi::Module {
    ...
    wasmi::Module::new(&engine, &wasm[..])
        .unwrap_or_else(|error| panic!("could not parse Wasm module from file {}: {}", file_name, error))
}

/// Parses the Wasm binary from the given `file_name` into a `runtime` module.
///
/// # Note
///
/// This includes validation and compilation to `runtime` bytecode.
...
/// Parses the Wasm source from the given `.wat` bytes into a `runtime` module.
///
/// # Note
///
/// This includes validation and compilation to `runtime` bytecode.
