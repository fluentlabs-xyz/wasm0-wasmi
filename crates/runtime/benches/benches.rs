...
use wazm_core::{Pages, ValueType, F32, F64};
use wazm_wasmi::{core::TrapCode, Engine, Extern, Func, Linker, Memory, Module, Store, Value};
...
const WASM_KERNEL: &str = "benches/wasm/wasm_kernel/target/wasm32-unknown-unknown/release/wasm_kernel.wasm";
    ...
    c.bench_function(&bench_id, |b| {
        ...
        linker
            .define(
                ...
                Func::wrap(
                    &mut store,
                    |_0: i32, _1: i32, _2: i64, _3: i32, _4: i32, _5: i32, _6: i32, _7: i32| -> i32 {
                        unimplemented!()
                    },
                ),
            )
        ...
        linker
            .define(
                "seal0",
                "seal_deposit_event",
                Func::wrap(&mut store, |_0: i32, _1: i32, _2: i32, _3: i32| unimplemented!()),
            )
            .unwrap();
        linker
            .define(
...
fn bench_execute_global_bump(c: &mut Criterion) {
    ...
    c.bench_function("execute/global_bump", |b| {
        ...
        let count_until = instance.get_export(&store, "bump").and_then(Extern::into_func).unwrap();
        let mut result = Value::I32(0);

        b.iter(|| {
            count_until
                .call(&mut store, &[Value::I32(BUMP_AMOUNT)], slice::from_mut(&mut result))
                .unwrap();
...
fn bench_execute_global_const(c: &mut Criterion) {
    ...
    c.bench_function("execute/global_const", |b| {
        ...
        let count_until = instance.get_export(&store, "call").and_then(Extern::into_func).unwrap();
        let mut result = Value::I32(0);

        b.iter(|| {
            count_until
                .call(&mut store, &[Value::I32(LIMIT)], slice::from_mut(&mut result))
                .unwrap();
...
fn bench_execute_recursive_ok(c: &mut Criterion) {
    ...
    c.bench_function("execute/recursive_ok", |b| {
        ...
        b.iter(|| {
            bench_call
                .call(&mut store, &[Value::I32(RECURSIVE_DEPTH)], slice::from_mut(&mut result))
                .unwrap();
...
fn bench_execute_recursive_scan(c: &mut Criterion) {
    ...
    const RECURSIVE_SCAN_EXPECTED: i32 = ((RECURSIVE_SCAN_DEPTH * RECURSIVE_SCAN_DEPTH) + RECURSIVE_SCAN_DEPTH) / 2;
    c.bench_function("execute/recursive_scan", |b| {
        let (mut store, instance) = load_instance_from_wat(include_bytes!("wat/recursive_scan.wat"));
        let bench_call = instance.get_export(&store, "func").and_then(Extern::into_func).unwrap();
        let mut result = Value::I32(0);
...
fn bench_execute_recursive_trap(c: &mut Criterion) {
    c.bench_function("execute/recursive_trap", |b| {
        let (mut store, instance) = load_instance_from_wat(include_bytes!("wat/recursive_trap.wat"));
        let bench_call = instance.get_export(&store, "call").and_then(Extern::into_func).unwrap();
        let mut result = [Value::I32(0)];
        b.iter(|| {
...
fn bench_execute_recursive_is_even(c: &mut Criterion) {
    c.bench_function("execute/recursive_is_even", |b| {
        ...
        b.iter(|| {
            bench_call
                .call(&mut store, &[Value::I32(50_000)], slice::from_mut(&mut result))
                .unwrap();
        });
...
fn bench_execute_host_calls(c: &mut Criterion) {
    c.bench_function("execute/host_calls", |b| {
        ...
        let instance = linker
            ...
            .ensure_no_start(&mut store)
            .unwrap();
        let call = instance.get_export(&store, "call").and_then(Extern::into_func).unwrap();
...
fn bench_execute_memory_sum(c: &mut Criterion) {
    c.bench_function("execute/memory_sum", |b| {
        ...
        let mut result = Value::I64(0);
        b.iter(|| {
            sum.call(&mut store, &[Value::I32(len as i32)], slice::from_mut(&mut result))
                .unwrap();
        });
...
fn bench_execute_memory_fill(c: &mut Criterion) {
    c.bench_function("execute/memory_fill", |b| {
        ...
        let params = [Value::I32(ptr as i32), Value::I32(len as i32), Value::I32(value as i32)];
        b.iter(|| {
...
fn bench_execute_vec_add(c: &mut Criterion) {
    ...
    c.bench_function("execute/memory_vec_add", |b| {
        let (mut store, instance) = load_instance_from_wat(include_bytes!("wat/memory-vec-add.wat"));
